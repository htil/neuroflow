...
class ldaProjectComponent extends Rete.Component {
    ...
    builder(node) {
        var in1 = new Rete.Input('in1', "ldaParams", sockets.Object);
        var in2 = new Rete.Input('in2', "point", sockets.Object);
        var out1 = new Rete.Output('out1','prediction',sockets.Object);
        return node.addOutput(out1).addInput(in1).addInput(in2);
    }

    worker(node, inputs, outputs) {
        if(inputs['in1'].length === 1){ 
                outputs['out1']=bci.ldaProject(inputs['in1'][0],inputs['in2'][0]);
        }     
    }
}

export default class ldaProject{
  ...
  async initialize() {
      var component = new ldaProjectComponent();
   ...
   static async create(editor,engine,window) {
      const o = new ldaProject(editor,engine);
