 ...
 *
 */
export let FlowPlotBlock = new CustomBlock(
  "flow_plot",
  (b: Blockly.Block) => {
  ...
  (b: Blockly.Block) => {
    let select = b.getFieldValue("plot_now"); // get the input value
...
export let FlowCategoryCallback = (ws: Blockly.Workspace): Array<Node> => {
  ...
  let button_text = unwind(
    [<Button>{ text: locale.flow.add, callbackKey: "add_flow" }],
    true
  );
  ...
  // Add plot block
  let flow_text = unwind([`${FlowPlotBlock.name}`], true);
  let flow = Blockly.Xml.textToDom(flow_text).firstChild;
  res.push(flow);
...
export class FlowMutator extends Blockly.Mutator {
  ...
  createEditor_() {
    ...
    // Sanity check
    if (info == undefined) throw "INVALID FLOW BLOCK";
  ...
  resizeBubble_() {
    ...
    if (
      Math.abs(this.workspaceWidth_ - width) > doubleBorderWidth ||
