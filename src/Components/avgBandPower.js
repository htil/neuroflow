...
class avgBandPowerComponent extends Rete.Component {

    constructor(){
        super("averageBandPower");
    ...
    worker(node, inputs, outputs) {
        if(inputs['in1'].length === 1 && inputs['in2'].length ===1){
            ...
            psd = bci.averageBandPowers(inputs['in1'][0], inputs['in2'][0], ['delta','theta','alpha', 'beta','gamma'],inputs['in3'][0]);
            else
            psd = bci.averageBandPowers(inputs['in1'][0], inputs['in2'][0], ['delta','theta','alpha', 'beta','gamma']);
            for(var i=0;i<5;i++){
...
export default class avgBandPower{
    constructor(editor,engine){
        this.editor = editor;
        this.engine = engine;
    }
    async initialize() {
        var component = new avgBandPowerComponent();
     ...
     static async create(editor,engine,window) {
        const o = new avgBandPower(editor,engine);
